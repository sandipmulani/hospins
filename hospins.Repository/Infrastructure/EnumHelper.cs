...
namespace hospins.Repository.Infrastructure
{
    public static class EnumHelper
    {
        ...
        public static string ToEnumValue(this string EnumName, object value)
        {
            try
            {
                var _enum = Type.GetType("hospins.Repository.Infrastructure." + EnumName, true);
                return ((Enum)Enum.ToObject(_enum, value)).ToDisplayName();
            }
            catch (Exception ex)
            {
                ex.SetLog("AuditTrail/ToEnumValue");
